...
# Step 8: Service Configuration and Startup ---
echo "Configuring and starting required systemd services..."

# Define an array of services that should be enabled and running
REQUIRED_SERVICES=(
  "acpid.service"
...
echo "Enabling all required services for next boot..."

for SERVICE_NAME in "${REQUIRED_SERVICES[@]}"; do
  echo "Ensuring service is enabled: ${SERVICE_NAME}"

  # Use 'systemctl is-enabled' to check if it's already enabled.
  # The '|| true' prevents the script from exiting if the service name is invalid or not installed.
  # The '2>/dev/null' suppresses error messages for missing services.

  if ! sudo systemctl is-enabled "${SERVICE_NAME}" &>/dev/null; then

    # Check if the service file exists before trying to enable
    if sudo systemctl list-unit-files "${SERVICE_NAME}" &>/dev/null; then
      sudo systemctl enable "${SERVICE_NAME}"
      echo "-> ${SERVICE_NAME} enabled successfully."
    else
      echo "-> WARNING: ${SERVICE_NAME} unit file not found. Skipping enable."
    fi
...
# -------------------------------------------
# Step 10: Final Reboot with Countdown
# -------------------------------------------
REBOOT_DELAY=10

echo "System will now reboot to apply all changes."

# Countdown loop
for ((i = ${REBOOT_DELAY}; i > 0; i--)); do
  printf "System rebooting in %2d seconds... Press Ctrl+C to cancel.\r" "$i"
  sleep 1
done

echo "" # Add a newline after the countdown finishes
sudo reboot
